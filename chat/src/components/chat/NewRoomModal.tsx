...
interface Props {
  ...
  addNewRoom: (newRoom: {roomName: string; roomDescription: string}) => void
...
const NewRoomModal: React.FC<Props> = ({showNewRoomModal, addNewRoom, closeNewRoomModal}) => {
  ...
  const handleRoomCreation = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    addNewRoom({roomName, roomDescription})
    setRoomName("")
  }
        ...
        >
          <div className="bg-gradient-to-br from-pink-500 via-purple-500 to-indigo-500 max-w-lg w-full p-6 rounded-md">
            <form className="flex flex-col gap-4" onSubmit={handleRoomCreation}>
              <h1 className="text-3xl text-center">New Room</h1>
              <TextField
                ref={inputRef}
              ...
              <nav>
                <ul className="flex justify-between gap-4">
                  <li className="w-full">
                    <button
                      className="w-full px-4 py-2 text-lg text-center bg-teal-500 rounded-md hover:bg-teal-600 transition-color duration-300 ease-in font-medium"
                      type="submit"
                    >
                      Create
                    </button>
                  ...
                  <li className="w-full">
                    <button
                      className="w-full px-4 py-2 text-lg text-center bg-rose-500 rounded-md hover:bg-rose-700 transition-color duration-300 ease-in font-medium"
                      type="button"
